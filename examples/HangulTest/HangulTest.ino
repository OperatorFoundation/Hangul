...
#define INDENTED_BOX 18
#define INDENTED_MESSAGE 32

#define SCREEN_WIDTH 400 // display width, in pixels
#define INDENTED_SCREEN_WIDTH 395 // to create a box slightly less wide than the display screen width
...
{ 
  ...
  u8g2Display.setFont(u8g2_font_unifont_t_korean2);  
...
void wrapNoSpaces(int indexStart){
    ...
    for(int index=indexStart; index <= composeBuffer.length(); index++){     
      ...
      if(wordSize+positionOnLine > maxCharPerLine){//if we exceed the symbol limit on 
        ...
        u8g2Display.println(currentLineBuffer);
        result += currentWord;
