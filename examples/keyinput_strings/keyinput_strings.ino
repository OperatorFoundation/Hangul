//this example uses a arduino (so c++) wrapper to instanciate 
//long key-input buffers as Strings, therefore allowing us to 
//pass single objects a growing string) to the renderer. this
//was my first attempt at rendering a buffer of text. 
//ToDo: handle spacebar presses. 
...
String generateLocalBuffer1(char keystroke)
{
  char arduinoChar[32];
...
void loop()   
{
   if (Serial.available() > 0){//checks for user input
     ...
     String composeBuffer = generateLocalBuffer1(textInputChar);
